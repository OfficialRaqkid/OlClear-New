<x-master-layout  
    :breadcrumbs="['VP Academic', 'Clearance Requests']"
    sidebar="dashboard.vp_academic.partials.sidebar"
    navbar="dashboard.vp_academic.partials.navbar"
    footer="dashboard.student.partials.footer">

<div class="az-dashboard-one-title mb-4">
    <div>
        <h2 class="az-dashboard-title">Academic Clearance Review</h2>
        <p class="az-dashboard-text">
            Review and approve academic clearance requests.
        </p>
    </div>
</div>
    ...
    <div class="card-body p-0">
        ...
        @if ($requests->isEmpty())
            <div class="text-center py-5 text-muted">
                No clearance requests available.
            </div>
        @else
                ...
                @foreach ($requests as $key => $req)
                    ...
                    <td class="text-center">
                        ...
                        @else
                            {{-- APPROVE --}}
                            <form
                                action="{{ route('vp_academic.clearances.accept', $req->id) }}"
                                method="POST"
                                class="d-inline">
                                @csrf
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    $('#holdModal').on('show.bs.modal', function (event) {
        ...
        document.getElementById('holdForm').action =
            "/vp_academic/clearance-requests/" + id + "/hold";
    });
    ...
    $('#viewModal').on('show.bs.modal', function (event) {
        ...
        document.getElementById('viewApproveForm').action =
            "/vp_academic/clearance-requests/" + id + "/approve";
    });
